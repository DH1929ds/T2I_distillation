...
import diffusers
from diffusers import AutoencoderKL, DDPMScheduler, StableDiffusionPipeline, StableDiffusionXLPipeline, UNet2DConditionModel, EulerDiscreteScheduler
...
def parse_args():
    ...
    parser.add_argument("--safe_tensor", action='store_true', help='safe_tensor instead pt')
    ...
    parser.add_argument("--unet_config_name", type=str, default="bk_small", 
        choices=["bk_base", "bk_small", "bk_tiny","original", "KOALA1B", "KOALA700M"])     
...
def main():
    ...
    train_dataset = x0_dataset(data_dir=args.train_data_dir, extra_text_dir=args.extra_text_dir,n_T=noise_scheduler.num_train_timesteps, 
                               ...
                               max_extra_text_samples=args.max_extra_text_samples, safe_tensor=args.safe_tensor)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                prompt_embeds, pooled_prompt_embeds = SDXL_encode_prompt(
                    unet,
                    text_encoder,
                    text_encoder_2,
                    batch['input_ids'],
                ...
                add_time_ids = add_time_ids.repeat(batch_size, 1).to(accelerator.device)
                
                add_text_embeds = pooled_prompt_embeds.to(accelerator.device)
